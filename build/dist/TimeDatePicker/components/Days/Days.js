...
const Days = () => {
    const { state, utils, options = defaultOptions, onDateChange, } = useCalendar();
    ...
    return (<View style={[style.container, { flexDirection: "row" }]} onLayout={changeItemHeight}>
      {days.map((day, index) => {
            let isSameDay = false;
            let isToday = false;
